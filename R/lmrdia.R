"lmrdia" <- function(usrtrim=FALSE) {
   step <- 0.001
   t3s  <- seq(-1, 1, step)
   nrow <- length(t3s)

   n <- 1
   aep4 <- matrix(nrow=nrow, ncol=2)
   for(t3 in t3s) {
     a <- abs(t3)
   ...
   n <- 1
   lim <- matrix(nrow=nrow, ncol=2)
   gpa <- matrix(nrow=nrow, ncol=2)
   for(t3 in t3s) {
     lim[n,1] <- t3
   ...
   n <- 1
   ksA <- seq(-1, 1, step)
   ksB <- seq(1+step, 20, 0.01)
   gev <- matrix(nrow=length(ksA)+length(ksB), ncol=2)
   for(k in ksA) {
     h <- -k
   ...
   n <- 1
   ks <- seq(-1, 1, step)
   glo <- matrix(nrow=length(ks), ncol=2)
   for(k in ks) {
     glo[n,1] <- -k
     glo[n,2] <- (1 + 5*k^2)/6
     n <- n + 1
   ...
   pdq3 <- matrix(nrow=nrow, ncol=2)
   pdq3[,1] <- t3s
   pdq3[,2] <- t4pdq3(t3s)

   n <- 1
   gov <- matrix(nrow=nrow, ncol = 2)
   for(t3 in t3s) {
     gov[n,1] <- t3
   ...
   tau34list <- list(limits=lim, aep4=aep4, cau=cau, exp=exp,
  ...
  if(usrtrim) {
    for(d in names(tau34list)) {
      t3lim <- par()$usr[1:2]; t4lim <- par()$usr[3:4]
      t3t4  <- tau34list[[d]]
      kt3  <- ncol(t3t4)-1; kt4 <- ncol(t3t4) # last two columns (code parallels lmrdia46 in this regard)
      wnt <- t3lim[1] <= t3t4[,kt3] & t3t4[,kt3] <= t3lim[2]
      t3t4[! wnt, kt3] <- NA
      wnt <- t4lim[1] <= t3t4[,kt4] & t3t4[,kt4] <= t4lim[2]
      t3t4[! wnt, kt4] <- NA
      tau34list[[d]] <- t3t4
    }
  }
